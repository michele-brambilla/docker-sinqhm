...
int init_filler(void)
{
  ...
  else
  {
    ...
    rt_printk ("init_module(): Debug  SHM allocated: dbg_size     =%9d bytes (shm_dbg_ptr   = %p)\n",dbg_size ,shm_dbg_ptr);
  ...
  dbg_printf(DBGMSG_INFO1, "init_module(): Debug  SHM allocated: dbg_size     =%9d bytes (shm_dbg_ptr   = %p)\n", dbg_size, shm_dbg_ptr);
  ...
  else
  {
    ...
    dbg_printf(DBGMSG_INFO1, "init_module(): Histo  SHM allocated: hst_size     =%9d bytes (shm_histo_ptr = %p)\n", hst_size, shm_histo_ptr);
    rt_printk ("init_module(): Histo  SHM allocated: hst_size     =%9d bytes (shm_histo_ptr = %p)\n", hst_size, shm_histo_ptr);
  }
  ...
  else
  {
    ...
    dbg_printf(DBGMSG_INFO1, "init_module(): Config SHM allocated: SHM_CFG_SIZE =%9d bytes (shm_cfg_ptr   = %p)\n", SHM_CFG_SIZE, shm_cfg_ptr);
    rt_printk ("init_module(): Config SHM allocated: SHM_CFG_SIZE =%9d bytes (shm_cfg_ptr   = %p)\n", SHM_CFG_SIZE, shm_cfg_ptr);
  }
...
void periodic_zeromq_rt_task(int t) {
  config_zeromq_loop();
}
...
void daq_loop_0mq() {
  ...
  while( shm_cfg_ptr[CFG_SRV_DO_DAQ_CMD] )
  {
    ...
    if (shm_cfg_ptr[CFG_SRV_RST_PRINT_BUFF_CMD] && !shm_cfg_ptr[CFG_FIL_RST_PRINT_BUFF_ACK]) {
      ...
      if (shm_cfg_ptr[CFG_FIL_RST_PRINT_BUFF_ACK] && !shm_cfg_ptr[CFG_SRV_RST_PRINT_BUFF_CMD]) {
	/* acknowledge is set, but Cmd not, so reset ack */
	shm_cfg_ptr[CFG_FIL_RST_PRINT_BUFF_ACK]=0;
      }
    // #endif
